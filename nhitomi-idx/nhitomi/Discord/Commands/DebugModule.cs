...
namespace nhitomi.Discord.Commands
{
    ...
    {
        [Command("debug"), Name("debug")]
        public Task DebugAsync() => Context.SendAsync<DebugMessage>();
    }

    public class DebugMessage : ReplyMessage
    {
        ...
        public DebugMessage(nhitomiCommandContext context, IDiscordClient discord, IDiscordMessageHandler messageHandler, IDiscordReactionHandler reactionHandler, MemoryInfo memory, IInteractiveManager interactive)
        {
            _context         = context;
            _discord         = discord;
