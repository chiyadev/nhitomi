...
function updateGuildsMetric(): void {
  guildCount.set(Discord.guilds.cache.size);
}
...
`.trim(),
    ...
    // scan message for links (this is slightly different from n!get command because strict=false; allows multiple links)
      ...
      const result = await Api.book.getBooksByLink({
        strict: false,
...
Discord.on(
  "messageDelete",
  wrapHandler("messageDelete", async (message) => {
...
(async (): Promise<void> => {
  ...
  if (!die)
    try {
      ...
      die = true;
      console.error("could not initialize api client", e);
    }
  ...
  if (die) setTimeout(() => process.exit(1), 5000);
})();
