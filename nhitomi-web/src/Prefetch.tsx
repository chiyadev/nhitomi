...
function getRetainedScroll(key: string) {
  ...
  if (isNaN(value)) value = parseInt(sessionStorage.getItem("scroll") || "");
...
function beginRetainedScrollTo(key: string, top: number, retry = 0) {
  ...
  requestAnimationFrame(() => {
    ...
    if (retry < 5) beginRetainedScrollTo(key, top, retry + 1);
  });
}

export type PrefetchMode = "prefetch" | "postfetch";

export type PrefetchGenerator<T, U extends {} = {}> = (
  x: U & { mode: PrefetchMode }
) => {
...
export function usePrefetch<T, U extends {}>(
  generator: PrefetchGenerator<T, U>,
  options: U
): [string, (mode?: NavigationMode) => Promise<T | undefined>] {
  ...
  const run = useCallback(
    async (mode: NavigationMode = "push") => {
      ...
      try {
        ...
        if (navigator.path !== startingPath) return;
...
export function useDynamicPrefetch<T, U extends {}>(
  generator: PrefetchGenerator<T, U>
): [ReactNode, (options: U, mode?: NavigationMode) => Promise<T | undefined>] {
  ...
  const node = useMemo(() => {
    ...
    return <PrefetchExecutorNode {...pending} />;
  }, [pending]);

  const run = useCallback(
    (options: U, mode?: NavigationMode) =>
...
const PrefetchExecutorNode = <T, U extends {}>({
  generator,
  options,
  mode,
  resolve,
  reject,
}: PendingDynamicPrefetch<T, U>) => {
  const [, run] = usePrefetch(generator, options);

  useAsync(async () => {
    try {
      resolve(await run(mode));
    } catch (e) {
      reject(e);
    }
  }, []);
...
export function usePostfetch<T, U extends {}>(
  generator: PrefetchGenerator<T, U>,
