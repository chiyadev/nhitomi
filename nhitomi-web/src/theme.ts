...
export class Color {
  ...
  get hex() {
    return SupportsHex8 ? this.color.toHex8String() : this.color.toHexString();
  }
  ...
  mix(other: Color, amount = 0.5) {
    return createColor(this.color.clone().mix(other.color, amount * 100));
  }
  ...
  tint(value: number) {
    return this.mix(getColor("white"), value);
  }

  shade(value: number) {
    return this.mix(getColor("black"), value);
  }
}
...
export function getColor(
  ...
  switch (color) {
    ...
    case "black":
      return createColor(colors.black);
    case "transparent":
      return createColor("transparent");
  }
...
export function createColor(color: ColorInput) {
  if (color instanceof TinyColor) return new Color(color);
  else return new Color(new TinyColor(color));
}
...
export function convertHex(hex: string, alpha?: number) {
  ...
  if (typeof alpha === "undefined") return `rgba(${r}, ${g}, ${b})`;
