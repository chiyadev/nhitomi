...
export const SearchInput = ({ result, className }: { result: BookSearchResult; className?: string }) => {
  ...
  const setTextWithSearch = useCallback(
    (text: string) => {
      setText(text);
  ...
  const placeholder = useLocalized("pages.bookListing.search", {
    total: result?.total,
  });
      ...
      <Suggestor tokens={tokens} inputRef={inputRef} setText={setTextWithSearch}>
        <div className="flex-grow relative overflow-hidden">
          <input
            ref={inputRef}
            className={cx(
              "pl-4 w-full h-full absolute top-0 left-0 border-none",
              css`
                background: none;
                color: transparent;
                caret-color: black;
  ...
  return (
    <div className="relative">
      <animated.div
        ...
        className={cx(
          "absolute right-0 bg-white h-full flex items-center z-10 px-4 cursor-pointer select-none",
...
const Suggestor = ({
  ...
  const token = useMemo(() => {
    if (typeof index === "number")
      return tokens
        .slice()
        .reverse()
        .find((token) => token.display && token.begin <= index);
  }, [index, tokens]);
  ...
  const complete = useCallback(() => {
    ...
    if (text[caret] !== " ") text = insert(text, caret, " ");
...
const SuggestorSection = ({
  ...
  setSelected: Dispatch<SuggestItem>;
}) => {
